(function (window) {
  function HTMLMagnifier(options) {
    ...
    const magnifierTemplate = `<div class="magnifier" style="display: none;position: fixed;overflow: hidden;background-color: white;border: 1px solid #555;border-radius: 12px;z-index:10000;">
    ...
    function syncViewport() {
      ...
      const x2 = getScrollLeft();
      const y2 = getScrollTop();
    ...
    function makeDraggable(ctrl, options) {
      ...
      function downHandler(e) {
        ...
        if (target && options.exclude.indexOf(target.tagName.toUpperCase()) == -1) {
          if (!parent || options.exclude.indexOf(parent.tagName.toUpperCase()) == -1) {
            ...
            pos_x = pageX - (dragObject.getBoundingClientRect().left + getScrollLeft());
            pos_y = pageY - (dragObject.getBoundingClientRect().top + getScrollTop());
      ...
      function moveHandler(e) {
        if (dragObject !== null) {
          const pageX = e.pageX || e.touches[0].pageX;
          const pageY = e.pageY || e.touches[0].pageY;
          const left = pageX - pos_x - ofs_x - getScrollLeft();
          const top = pageY - pos_y - ofs_y - getScrollTop();
